// Command silo-plugin-audiobooks is the audiobooks portal plugin
...
import (
	...
	"github.com/RXWatcher/silo-plugin-audiobooks/internal/consumer"
	"github.com/RXWatcher/silo-plugin-audiobooks/internal/event"
	"github.com/RXWatcher/silo-plugin-audiobooks/internal/httproutes"
	...
	"github.com/RXWatcher/silo-plugin-audiobooks/internal/recommend"
	pluginrt "github.com/RXWatcher/silo-plugin-audiobooks/internal/runtime"
	"github.com/RXWatcher/silo-plugin-audiobooks/internal/scheduler"
	...
	"github.com/RXWatcher/silo-plugin-audiobooks/web"
)
...
func main() {
	logger := hclog.New(&hclog.LoggerOptions{Name: "silo-plugin-audiobooks"})
	...
	hostBase := os.Getenv("SILO_HOST_URL")
	if hostBase == "" {
	...
	// sticky-session-aware load balancer, set SILO_REDIS_URL so
	...
	rt := pluginrt.New(manifest, func(cfg pluginrt.Config) error {
		...
		absHandler := abs.NewHandler(abs.Deps{
			...
			InstallID:     func() string { return "silo.audiobooks" },
...
// hub. When SILO_REDIS_URL is set, we wire a Redis adapter so events
...
func buildABSSocketOptions(logger hclog.Logger) *abssocket.Options {
	raw := strings.TrimSpace(os.Getenv("SILO_REDIS_URL"))
	if raw == "" {
		return nil
	}
	opts, err := goredis.ParseURL(raw)
	if err != nil {
		logger.Warn("SILO_REDIS_URL is set but unparseable; falling back to in-memory Socket.io adapter",
			"err", err.Error())
		return nil
	}
